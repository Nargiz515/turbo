...
function show(){
  ...
  //                   <img class="w-[100%] h-[100%] object-cover" src="${index.images}" alt="">
  //               </div>
  //               <div">
  //                   <p class="font-bold text-lg">${index.price}</p>
  //                   <p class="font-medium text-md">${index.brand}</p>
  //                   <p>${index.year}</p>
  //                   <p class="text-[#8d94ad]">${index.city}</p>
  //               </div> 
  //   </div>`
  // }
  data.map((item)=>{
    cards.innerHTML +=`<div class="border-[2px] rounded-xl">
      <div class="h-60 w-full gap-[20px]">
                     <img class=" w-[100%] rounded-xl h-[100%] object-cover" src="${item.images}" alt="">
                 </div>
                 <div">
                     <p class="font-bold text-lg">${item.price}</p>
                     <p class="font-medium text-md">${item.brand}</p>
                     <p>${item.year}</p>
                     <p class="text-[#8d94ad]">${item.city}</p>
                 </div> 
     </div>`
  })
}
